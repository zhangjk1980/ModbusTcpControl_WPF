    ...
    public partial class frmClient : Form
    {
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var startAddress = short.Parse(this.startAddress.Text);
                var num = short.Parse(this.regNum.Text);
                var data = CoilWrite(21, true);//HoldRegisterRead(startAddress,num);

                // 发送
                //if (client.Send(bytes, bytes.Length))
                if (client.Send(data, data.Length))
                {
                    AddMsg(string.Format("$ ({0}) Send OK --> {1}", connId, System.Text.Encoding.Default.GetString(data)));
                }
                else
                {
                    AddMsg(string.Format("$ ({0}) Send Fail --> {1} ({2})", connId, System.Text.Encoding.Default.GetString(data), data.Length));
                }
        ...
        /// </summary>
        /// <returns></returns>
        public byte[] HoldRegisterRead(short registerAdd, short num)
        {
            try
            {
                //[0]:填充0，清掉剩余的寄存器
                ...
                values.AddRange(ValueHelper.Instance.GetBytes(++dataIndex));//1~2.(Transaction Identifier)
        ...
        /// </summary>
        /// <returns></returns>
        public byte[] CoilWrite(short registerAdd, bool open)
        {
            try
            {
                //[0]:填充0，清掉剩余的寄存器
        ...
        HandleResult OnReceive(TcpClient sender, byte[] bytes)
        {
            ...
            AddMsg(string.Format(" > [{0},OnReceive] -> {1}:({2} bytes)", sender.ConnectionId, System.Text.Encoding.Default.GetString(bytes), bytes.Length));
